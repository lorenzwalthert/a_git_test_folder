...
```
## Basic navigation - Commandline 101
```{r}

help # some basic information

ls # show all elements in directory
ls -la # structure the content a bit more
...
rm -r testfolder # -r for recursive for removing folders
...
## Inspecting changes
```{r}
git diff # inspect differences between HEAD and local file (= inspect unstagged changes).
git diff master..branch1 -- text1.txt # inspect all the differences between two branches (master and branches).
